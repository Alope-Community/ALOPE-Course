    ...
    return (
        ...
        >
            <div className="relative h-60 w-full">
                <img src={props.cover} alt="cover" className="h-full w-full" />
            </div>
            <div className="flex flex-1 flex-col justify-between p-4">
                <div>
                    <div className="mb-2 flex items-center gap-3 text-xs text-black">
                        <Icon icon={<IconCalendar />} label={formatDate(props.created_at || '')} />

                        <Icon icon={<IconClock />} label={diffForHumans(props.created_at || '')} />

                        <Icon icon={<IconEye />} label={props.reads?.length + " Views"} />
                    </div>
